...
void xAna_Zpt_optimization_signal(string inputtxtFilename, string outputtxtFilename)
...
//void xAna_bkg_ztoee_forCheckSkimmedTree(string inputFilename = "../DYJetsToLL_M-50_HT-70to100_TuneCP5_PSweights_13TeV-madgraphMLM-pythia8.root", string outputfile = "outputEffCheck_DYJetsToLL_M-50_HT-70to100_TuneCP5_PSweights_13TeV-madgraphMLM-pythia8.root")
{
    ...
    TH1D* h_Zpt_after_preselection = new TH1D("h_Zpt_after_preselection", "Z pT After Preselection", 100, 0, 1000);
    h_Zpt_after_preselection->Sumw2();

    TH1D* h_before_Zpt_signal = new TH1D("h_before_Zpt_signal", "nEvents before any Zpt cut", 5, 0, 5);
    h_before_Zpt_signal->Sumw2();

    TH1D* h_after_Zpt50_signal = new TH1D("h_after_Zpt50_signal", "After Z pT cut >50 GeV", 5, 0, 5);
    h_after_Zpt50_signal->Sumw2();

    TH1D* h_after_Zpt60_signal = new TH1D("h_after_Zpt60_signal", "After Z pT cut >60 GeV", 5, 0, 5);
    h_after_Zpt60_signal->Sumw2();

    TH1D* h_after_Zpt100_signal = new TH1D("h_after_Zpt100_signal", "After Z pT cut >=100 GeV", 5, 0, 5);
    h_after_Zpt100_signal->Sumw2();

    TH1D* h_after_Zpt_scan_signal = new TH1D("h_after_Zpt_scan_signal", "After Z pt cut scanning", 100, 0, 1000);
    ...
    Float_t f_ZbosonPt_afterAlpha3D;
    Float_t f_ZbosonMass_afterAlpha3D;
    Float_t f_ZbosonEta_afterAlpha3D;
    Float_t f_ZbosonPhi_afterAlpha3D;
    ...
    tree->Branch("f_ZbosonPt_afterAlpha3D", &f_ZbosonPt_afterAlpha3D);
    tree->Branch("f_ZbosonMass_afterAlpha3D", &f_ZbosonMass_afterAlpha3D);
    tree->Branch("f_ZbosonEta_afterAlpha3D", &f_ZbosonEta_afterAlpha3D);
    tree->Branch("f_ZbosonPhi_afterAlpha3D", &f_ZbosonPhi_afterAlpha3D);
    ...
    vector<double> vec_signalEffLow;
    vector<double> vec_signalEffMed;
    vector<double> vec_signalEffHigh;
    ...
    h_Zpt_after_preselection->Reset();
    ...
    //TreeReader data(inputtxtFilename.c_str());
    TreeReader data(inputtxtFilename.data(), "outTree");
            ...
            if (recoeeEvent)
            {
                ...
                while (cutvalue<1000)
                {
                    ...
                    if (Zboson.Pt() > cutvalue)
                        h_after_Zpt_scan_signal->Fill(cutvalue, eventWeight);
                }

                if (Zboson.Pt() > 50)
                    h_after_Zpt50_signal->Fill(1, eventWeight);

                if (Zboson.Pt() > 60)
                    h_after_Zpt60_signal->Fill(1, eventWeight);

                if (Zboson.Pt() >= 100)
                    h_after_Zpt100_signal->Fill(1, eventWeight);
                ...
                TLorentzVector Zboson_afterAlpha3D = goodElectrons[0] + goodElectrons[1];
                float Zboson_pt_afterAlpha3D = Zboson_afterAlpha3D.Pt();
                float Zboson_mass_afterAlpha3D = Zboson_afterAlpha3D.M();
                float Zboson_eta_afterAlpha3D = Zboson_afterAlpha3D.Eta();
                float Zboson_phi_afterAlpha3D = Zboson_afterAlpha3D.Phi();
                ...
                f_ZbosonPt_afterAlpha3D = Zboson_pt_afterAlpha3D;
                f_ZbosonMass_afterAlpha3D = Zboson_mass_afterAlpha3D;
                f_ZbosonEta_afterAlpha3D = Zboson_eta_afterAlpha3D;
                f_ZbosonPhi_afterAlpha3D = Zboson_phi_afterAlpha3D;
                tree->Fill();
    ...
    h_Zpt_after_preselection->Write();
    h_before_Zpt_signal->Write();
    h_after_Zpt50_signal->Write();
    h_after_Zpt60_signal->Write();
    h_after_Zpt100_signal->Write();
    h_after_Zpt_scan_signal->Write();
    h_ee_npass->Write();
    ...
    if ((inputFile).find("wgen_Vector_MonoZLL_NLO_Mphi-500_Mchi2") != std::string::npos)
    {
        ...
        double signalEfflow = (h_low_Zpt_signal->Integral())/totalMCevents;

        double signalEffmedium = (h_med_Zpt_signal->Integral())/totalMCevents;

        double signalEffhigh = (h_high_Zpt_signal->Integral())/totalMCevents;

        //save signalEfflow, med, high in each list/vector
        vec_signalEffLow.push_back(signalEfflow);
        vec_signalEffMed.push_back(signalEffmedium);
        vec_signalEffHigh.push_back(signalEffhigh);
    }

    else
    {
        nEventsBkgLow = (h_low_Zpt_signal->Integral()) * 6077.22 * 40.5 * 1000 / totalMCevents;
        //sumnEventsBkgLow

        nEventsBkgMed = (h_med_Zpt_signal->Integral()) * 6077.22 * 40.5 * 1000 / totalMCevents;
        //sumnEventsBkgMed

        nEventsBkgHigh = (h_high_Zpt_signal->Integral()) * 6077.22 * 40.5 * 1000 / totalMCevents;
        //sumnEventsBkgHigh
    }*/
